...
exports.createCatalog = async (req, res) => {
  try {
    const { catalogName, categories, subCategories } = req.body;
    ...
    if (store.status !== "approved") {
      return res.status(403).json({ message: "Store must be approved before creating a catalog" });
    }
    ...
    res.status(201).json({ message: "Catalog submitted successfully", catalog });
  } catch (error) {
...
exports.editCatalog = async (req, res) => {
  try {
    const { catalogId, catalogName, categories, subCategories } = req.body;
    const files = req.files;
    ...
    // Find the catalog
    const catalog = await Catalog.findOne({ _id: catalogId, sellerId: req.user._id });
    if (!catalog) {
    ...
    const areOtherFieldsChanged =
      ...
      JSON.stringify(parsedSubCategories) !== JSON.stringify(catalog.subCategories) ||
...
exports.getCatalogStatus = async (req, res) => {
  try {
    const catalog = await Catalog.find({ sellerId: req.user._id }).select("status rejectionReasons");
...
exports.updateCatalogStatus = async (req, res) => {
  try {
    ...
    const catalog = await Catalog.findById(catalogId).populate("sellerId", "email");
    if (!catalog) {
      return res.status(404).json({ message: "Catalog not found" });
    }
    ...
    if (status === "rejected" && rejectionReasons) {
      ...
      await createNotification(
        ...
        "error",
        "catalog",
        `Your catalog ${catalog.catalogName} has been rejected. Reasons: ${rejectionReasons.map((r) => `${r.field}: ${r.reason}`).join(", ")}.`,
        "/seller/store/catalog-details",
    ...
    res.status(200).json({ message: `Catalog ${status} successfully`, catalog });
  } catch (error) {
...
exports.deleteCatalog = async (req, res) => {
  try {
    ...
    if (catalog.status === "pending") {
      return res.status(403).json({ message: "Cannot delete a catalog that is pending approval" });
    }
